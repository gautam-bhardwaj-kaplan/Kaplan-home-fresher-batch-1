...
router.get("/activity/:student_id/:topic_id", async (req, res) => {
  try {
    ...
    if (rows.length > 0) {
      ...
      res.json({ hours_studied: null, quiz_score: null, completion_date_topic: null });
    }
  } catch (err) {
...
router.post("/activity", async (req, res) => {
  try {
    const { student_id, topic_id,hours_studied, quiz_score, completion_date_topic } = req.body;
    ...
    res.status(500).json({ error: "Failed to save activity", details: err.message });
  }
});
